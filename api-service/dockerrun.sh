...
if [ -z "$Webhookvivian" ]; then
    Webhookvivian="pdex";
fi

docker run --restart=always -p 8080:5000 -d -v $PWD/data:/data -e Webhookvivian=$Webhookvivian -e postgreshost=$postgreshost -e postgresport=$postgresport -e postgresuser=$postgresuser -e postgrespwd=$postgrespwd -e postgresdb=$postgresdb --name incognito-analytic-api-service incognitochain/incognito-analytic:incognito-analytic-api-service